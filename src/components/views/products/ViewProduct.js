...
function ViewProduct() {
  ...
  return (
<Box sx={{ height: 400, margin: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <DataGrid
        rows={drinkArray}
        columns={[
          { field: 'drinks_name', headerName: 'Tên sản phẩm', width: 200 },
          { field: 'description', headerName: 'Mô tả', width: 200 },
          { field: 'drinks_image', headerName: 'Ảnh mô tả', width: 200, renderCell: (params) => <img src={params.value} alt="Drink" style={{ maxHeight: "50px" }} /> },
          { field: 'price', headerName: 'Giá', type: 'number', width: 150 },
          { field: 'rating', headerName: 'Đánh giá', type: 'number', width: 150 },
          { field: 'sale', headerName: 'Giảm giá(%)', type: 'number', width: 150 },
          { field: 'sold_count', headerName: 'Số lượng đã bán(cốc)', type: 'number', width: 200 },
            ...
            renderCell: (params) => (
              <>
                <Fab
                  ...
                  color="primary"
                  aria-label="update"
                  onClick={() => navigate(`/update-product/${params.row.drinksid}`)}
                >
                  <BuildIcon />
                </Fab>
                <Fab
